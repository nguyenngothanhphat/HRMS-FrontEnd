describe('Employee Directory', () => {
  ...
  const employee_email = 'sandeep@mailinator.com';
  ...
  it('Directory', () => {
    ...
    //           // FILTER
    cy.get('.CustomOrangeButton___qz7iA').click({ force: true }).wait(3000);
    ...
    cy.get('#filter_reportingManager')
      .click({ force: true })
      .type('Mark Gigliotti', { force: true })
      .click({ force: true }); // by reporting manager
    cy.wait(2000);
    cy.get('.ant-select-item-option-content').eq(0).click({ force: true });
