  ...
  <ExcelFileEx
    ...
    element={
      <div>
        ...
        <span className={styles.ml10}>EXCEL</span>
      </div>
    }
  >
    <ExcelSheetEx data={data} name={formatMessage({ id: 'excel.bill' }).toUpperCase()}>
      <ExcelColumnEx
        label={formatMessage({ id: 'bill.category' })}
        value={col => getCategory(col.category)}
      />
      <ExcelColumnEx label={formatMessage({ id: 'bill.type' })} value={col => getType(col)} />
      <ExcelColumnEx
        label={formatMessage({ id: 'bill.updatedAt' })}
        value={col => (col.date ? moment(col.date).format('YYYY-MM-DD') : '')}
      />

      <ExcelColumnEx
        label={formatMessage({ id: 'bill.group' })}
