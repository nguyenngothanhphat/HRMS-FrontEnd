...
class SearchLocation extends React.PureComponent {
  ...
  render() {
    ...
    const { delIconClassName } = this.props;
    ...
    return (
      <Row gutter={12}>
        <Col span={isFull ? 24 : 22}>
          <Select
            className={styles.select}
            showSearch
            value={placeID}
            placeholder={formatMessage({ id: 'mileage.searchLocationPlaceHolder' })}
        ...
        {!isFull && (
          <Col className={delIconClassName} span={2}>
            {delIcon}
          </Col>
        )}
      </Row>
