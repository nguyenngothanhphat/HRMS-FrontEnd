...
@connect(({ locations, loading }) => ({
  locations,
  saving: loading.effects['locations/saveItem'],
}))
@Form.create()
class New extends Component {
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({ type: 'locations/fetchItem' });
  }
  ...
  render() {
    ...
    const { code, nativeName, alpha3Code, name, currency: flagIcon, currency } = item || {};
    return (
