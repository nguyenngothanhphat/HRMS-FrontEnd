  ...
  deduplicate(listType)
    ...
    .forEach(item => {
      ...
      data.push([
        item,
        billOfType.length,
        `${numeral(roundNumber(amount, 2)).format('0,0[.]00')} ${currency}`,
      ]);
    });
  return data;
};
  ...
  bills
    ...
    .forEach(bill => {
      ...
      data.push([
        ...
        paymentOption,
        {
          content: `${numeral(roundNumber(bill.originAmount, 2)).format('0,0[.]00')} ${
            bill.originCurrency
          }`,
          colSpan: 1,
          rowSpan: 1,
        ...
        {
          content: `${numeral(roundNumber(bill.amount, 2)).format('0,0[.]00')} ${currency}`,
          colSpan: 1,
          rowSpan: 1,
          styles: { halign: 'left', fontStyle: 'bold' },
        },
...
const getPageHeader = async (
  ...
  doc.text(
    calcXOffset(doc, `${numeral(totalAmount).format('0,0[.]00')} ${currency}`),
    28,
    `${numeral(totalAmount).format('0,0[.]00')} ${currency}`
  );

  // status
  doc.setFontType('normal');
  doc.setFontSize(12);
  doc.text(calcXOffset(doc, status), 20, convertStatusText(status));

  doc.setFontSize(8);
  doc.setDrawColor(128, 128, 128);
  doc.line(15, 22, 196, 22);

  doc.text(15, 26, `${formatMessage({ id: 'pdf.report-id' })}: ${code}`);
  doc.text(
    15,
    30,
    ...
    doc.autoTable({
      ...
      body: [
        [
          ...
          `${numeral(projectSummaryData.totalAmount).format('0,0[.]00')} ${currency ||
            backupCurrency}`,
        ],
        [
          formatMessage({ id: 'pdf.reimbursable' }),
          `${numeral(dueCompany).format('0,0[.]00')} ${currency || backupCurrency}`,
        ],
        [
          formatMessage({ id: 'pdf.non-reimbursable' }),
          `${numeral(dueEmployee).format('0,0[.]00')} ${currency || backupCurrency}`,
        ],
      ],
      margin: { left: 106 },
    });
