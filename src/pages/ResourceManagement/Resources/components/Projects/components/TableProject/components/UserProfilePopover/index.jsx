...
const UserProfilePopover = (props) => {
  ...
  const { headQuarterAddress: { state = '', country: { name = '' } = {} } = {} } = location || {};
  ...
  const userInfo = () => {
    const getTimezone = getTimezoneViaCity(state) || getTimezoneViaCity(name) || '';
    const timezone =
    ...
    const items = [
      {
        ...
        value: <span className={styles.managerName}>{manager?.generalInfo?.legalName}</span>,
        link: manager?.generalInfo?.userId,
      },
      {
        label: 'Mobile',
