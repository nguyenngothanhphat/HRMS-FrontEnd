...
const TeamLeaveCalendar = (props) => {
  ...
  const checkDate = (date, listDate) => {
    return listDate.filter((val) => moment(val.date).format('MM/DD/YYYY') === moment(date).format('MM/DD/YYYY')) || []
  }
      ...
      <div>
        {list.map((member) => (
          <span style={{ display: 'block' }}>
            ...
            {member.employeeInfo.generalInfo.firstName} {member.employeeInfo.generalInfo.lastName} ({member ? member.duration[0].timeOfDay : ''})
          </span>
        ))}
        {/* <span style={{ display: 'block' }}>
...
export default connect(({dashboard: { teamLeaveRequestList = [] } = {}}) => ({teamLeaveRequestList}))(TeamLeaveCalendar);
